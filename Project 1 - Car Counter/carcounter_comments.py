...
mask = cv.resize(mask,(1920, 1080)) # karachi
# mask = cv.resize(mask,(1280, 720)) # car
...
while True:
    ...
    for r in result: # r has tensors in it
        ...
        for coord in boxes:
            ...
            cvzone.cornerRect(frame,bbox=bbox,l = 10 , t = 1 , rt =1,colorR = (0,0,0),colorC = (255,255,255)) # (0,165,255)
            ...
            # cv.putText(frame,f'{crossedline[0]} {crossedline[1]}',(crossedline[0],crossedline[1]),fontFace=cv.FONT_HERSHEY_COMPLEX,fontScale=1,color = (255,24,239))
            # cv.putText(frame,f'{crossedline[2]} {crossedline[3]}',(crossedline[2],crossedline[3]),fontFace=cv.FONT_HERSHEY_COMPLEX,fontScale=1,color = (255,24,239))

            # print(clsname)
            # putting the confidence level on the rectangle on the top left of the bbox

            # if (int(coord.cls[0]) == 2 or int(coord.cls[0]) == 3 or int(coord.cls[0]) == 5 or int(coord.cls[0]) == 7) and confidence >0.3:
    ...
    for t in trackres:
        ...
        if (crossedline[0] < midpointx < crossedline[2]) and crossedline[1] - 15 < midpointy < crossedline[3] + 15 :
            ...
            if id_list.count(id)==1:
                ...
                print('inside ifff','len is' , id_list.count(id) , id_list.count(id)==1)
                car_detect_count += 1
    cv.putText(frame,f'No. of cars:{car_detect_count}',(900,100),fontFace  = cv.FONT_HERSHEY_SIMPLEX,fontScale=1,color=(0,0,0),thickness=2)
